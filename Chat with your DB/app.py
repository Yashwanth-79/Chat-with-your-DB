...
radio_opt = ["Use SQLLite 3 Database- nammastore", "Connect to you MySQL Database"]
...
if radio_opt.index(selected_opt) == 1:
    ...
    mysql_port = st.sidebar.text_input("Provide MySQL Port", value="3306")
    mysql_user = st.sidebar.text_input("MYSQL User", value="root")
    mysql_password = st.sidebar.text_input("MYSQL password", type="password")
    mysql_db = st.sidebar.text_input("MySQL database", value="nammastore")
else:
    db_uri = LOCALDB
...
def configure_db(db_uri, mysql_host=None, mysql_port=None, mysql_user=None, mysql_password=None, mysql_db=None):
    if db_uri == LOCALDB:
        dbfilepath = (Path(__file__).parent / "nammastore.db").absolute()
        print(dbfilepath)
...
if db_uri == MYSQL:
    db = configure_db(db_uri, mysql_host, mysql_port, mysql_user, mysql_password, mysql_db)
else:
